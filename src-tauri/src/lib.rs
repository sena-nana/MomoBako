...
use repository_service::{
    ...
    PluginHookExecutionListRequest, PluginHookExecutionListResponse, PluginMutationResponse,
    RepositoryAction, RepositoryActionEnabledRequest, RepositoryActionMutationResponse,
    RepositoryActionRunRequest, RepositoryActionRunResponse,
...
async fn list_plugin_hook_executions(
    ...
    runtime
        .run_read(move |state| {
            state.list_plugin_hook_executions(request.unwrap_or_default())
        })
        .await
}
