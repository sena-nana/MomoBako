...
const LOCAL_FILESYSTEM_FILE_SEARCH_MODE_KEY: &str = "fileSearchMode";
...
enum LocalFileSearchMode {
    Recursive,
    Ntfs,
    Everything,
}

fn local_file_search_mode_from_config(config: &serde_json::Value) -> LocalFileSearchMode {
    match config
...
fn collect_repository_files_with_mode(
    repo_root: &Path,
    config: &serde_json::Value,
) -> Result<Vec<DiscoveredFile>, String> {
...
fn collect_repository_files_with_fallback(
    repo_root: &Path,
    label: &str,
...
fn collect_repository_files_everything(repo_root: &Path) -> Result<Vec<DiscoveredFile>, String> {
    collect_repository_files_everything_impl(repo_root)
}

#[cfg(windows)]
fn collect_repository_files_everything_impl(
    repo_root: &Path,
) -> Result<Vec<DiscoveredFile>, String> {
...
#[cfg(windows)]
fn collect_repository_files_ntfs(repo_root: &Path) -> Result<Vec<DiscoveredFile>, String> {
...
#[cfg(not(windows))]
fn collect_repository_files_ntfs(_repo_root: &Path) -> Result<Vec<DiscoveredFile>, String> {
    Err("NTFS file search is only available on Windows".to_string())
}

fn push_local_discovered_file(
    repo_root: &Path,
    path: &Path,
...
fn canonical_repository_index_root(repo_root: &Path) -> PathBuf {
    let root = repo_root
        .canonicalize()
...
fn path_is_under_repository_root(repo_root: &Path, path: &Path) -> bool {
    indexed_repository_relative_path(repo_root, path).is_ok()
}

fn indexed_repository_relative_path(repo_root: &Path, path: &Path) -> Result<String, String> {
    #[cfg(windows)]
    {
...
fn finalize_local_discovered_files(
...
fn call_builtin_local_filesystem(
    ...
    let mut config = payload
        ...
        .cloned()
        .unwrap_or_else(|| serde_json::json!({}));
    if let Some(file_search_mode) = plugin_config.get(LOCAL_FILESYSTEM_FILE_SEARCH_MODE_KEY) {
        if let Some(config_object) = config.as_object_mut() {
            config_object.insert(
                LOCAL_FILESYSTEM_FILE_SEARCH_MODE_KEY.to_string(),
                file_search_mode.clone(),
            );
        }
    }
    match method {
...
impl FileSystemBackendAdapter for LocalFileSystemBackend {
    ...
    fn list_files(
        ...
        collect_repository_files_with_mode(repo_root, config)
    }
